...
// 21. Waitlist Referral Card
export function LiveWaitlistReferralCard() {
  const { showToast } = useLibrary();
  const [isJoined, setIsJoined] = useState(false);
  const [copied, setCopied] = useState(false);
  const [claimedSlots, setClaimedSlots] = useState<number[]>([]);
  const totalSlots = 3;

  // Cat avatar from uploaded screenshot with fallback
  const catAvatar = "https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?w=120&auto=format&fit=crop&q=80";

  const handleJoin = () => {
    setIsJoined(true);
    setClaimedSlots([0]);
    showToast("You joined the Weblocks waitlist! Position #142", "success");
  };

  const handleCopy = () => {
    setCopied(true);
    if (claimedSlots.length < totalSlots) {
      setClaimedSlots((prev) => [...prev, prev.length]);
      showToast("Invite link copied & +1 friend unlocked!", "success");
    } else {
  ...
  const handleReset = (e: React.MouseEvent) => {
    ...
    setIsJoined(false);
    setClaimedSlots([]);
    showToast("Reset waitlist demo state", "info");
  };

  return (
    <div className="flex flex-col items-center justify-center p-4 select-none font-sans">
      {/* 1. Floating Brand Capsule matching screenshot */}
      <div className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-white/80 dark:bg-white/[0.06] border border-black/[0.08] dark:border-white/10 text-[11px] font-medium text-ink dark:text-white/80 mb-3 shadow-xs backdrop-blur-sm">
        {/* Cat silhouette icon */}
        <svg className="w-3.5 h-3.5 text-current shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d="M12 5c-4 0-7 3-7 7 0 2.5 1.5 4.5 3.5 5.8V21l3.5-1.5 3.5 1.5v-3.2c2-1.3 3.5-3.3 3.5-5.8 0-4-3-7-7-7Z" />
      ...
      {/* 2. Main Obsidian Waitlist Card */}
      <div className="relative w-full max-w-[340px] rounded-[26px] bg-[#121316] text-white p-5 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden transition-all">
        {/* Background Concentric Radar Rings radiating from "You" node */}
        <svg
          className="absolute inset-0 w-full h-full pointer-events-none opacity-20"
          xmlns="http://www.w3.org/2000/svg"
        >
          <circle cx="68" cy="60" r="32" fill="none" stroke="white" strokeWidth="1" strokeDasharray="3 3" />
          <circle cx="68" cy="60" r="64" fill="none" stroke="white" strokeWidth="1" />
          <circle cx="68" cy="60" r="98" fill="none" stroke="white" strokeWidth="1" />
          <circle cx="68" cy="60" r="136" fill="none" stroke="white" strokeWidth="1" />
          <circle cx="68" cy="60" r="178" fill="none" stroke="white" strokeWidth="1" />
          <circle cx="68" cy="60" r="224" fill="none" stroke="white" strokeWidth="1" />
        </svg>

        {/* Top Nodes Row: You -> Connector Line -> 3 Invite Slots */}
        <div className="relative z-10 flex items-center justify-between mb-6 pt-1">
          ...
          <div className="relative flex flex-col items-center justify-center p-2 rounded-2xl bg-white/[0.06] border border-white/15 w-[66px] h-[78px] backdrop-blur-md shadow-md">
            <div className="relative w-9 h-9 rounded-full overflow-hidden border border-white/25 bg-white/10 flex items-center justify-center mb-1 shrink-0 shadow-inner">
              <img
                src={catAvatar}
                alt="You"
          ...
          {/* Connector Line with pulsating dot */}
              ...
              <div
                className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                  isJoined
                    ...
                    : "bg-white/60 shadow-[0_0_6px_rgba(255,255,255,0.6)]"
                }`}
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-white/[0.02] border border-white/5">
            {Array.from({ length: totalSlots }).map((_, idx) => {
              const isClaimed = claimedSlots.includes(idx);
              return (
                <div
                  key={idx}
        ...
        <div className="relative z-10 flex flex-col gap-1.5 mb-5">
          <div className="flex items-center justify-between">
            <h3 className="text-[17px] font-semibold text-white tracking-tight leading-snug">
              {isJoined ? "You're in the waitlist" : "You're not in the waitlist"}
            </h3>
            {isJoined && (
              <button
                type="button"
          ...
          <p className="text-[12.5px] text-white/50 leading-relaxed max-w-[280px]">
            {isJoined
              ? "Share your referral link with collaborators to climb the queue and unlock instant component downloads."
...
// 24. Stacked Task Card
export function LiveStackedTaskCard() {
  const { showToast } = useLibrary();
  const [tasks, setTasks] = useState([
    ...
    {
      id: 2,
      title: "Review dark mode tokens",
      meta: "Design System • Assigned by Sarah 15m ago",
    },
    {
      id: 3,
      title: "Export interactive components",
      meta: "Weblocks Library • Ready for Review",
    },
    {
      id: 4,
      title: "Verify accessibility contrast",
      meta: "Production Gate • Due today",
    },
  ]);
  ...
  const handleMarkDone = () => {
    if (!activeTask || isSwiping) return;
    setIsSwiping(true);
    showToast(`Completed: "${activeTask.title}"`, "success");
    setTimeout(() => {
      setTasks((prev) => {
        const [first, ...rest] = prev;
        return [...rest, first]; // Cycle to back
      });
      setIsSwiping(false);
    }, 250);
  };
  ...
  const handleRemindLater = () => {
    if (!activeTask || isSwiping) return;
    setIsSwiping(true);
    showToast(`Reminding you later: "${activeTask.title}"`, "info");
    setTimeout(() => {
      setTasks((prev) => {
        const [first, ...rest] = prev;
        return [...rest, first]; // Shuffle to back
      });
      setIsSwiping(false);
    }, 250);
  };
...
export const LIVE_COMPONENTS_MAP: Record<string, React.ComponentType> = {
  ...
  "stacked-task-card": LiveStackedTaskCard,
};
