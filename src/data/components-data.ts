...
export function ShareModalDialog({
  ...
  return (
    <div className="relative w-full max-w-[340px] sm:max-w-[360px] rounded-[30px] bg-white dark:bg-[#161718] border border-black/[0.06] dark:border-[#23252a] shadow-[0_20px_50px_-15px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.6)] pt-8 pb-6 px-5 sm:px-6 select-none font-sans">
...
export function AIReasoningStream({
  ...
  },
  {
    ...
    props: [
      ...
      { name: "meta", type: "string", default: '"New Task • Assigned to You 3m ago"', description: "Task category and timestamp metadata" },
      { name: "totalInQueue", type: "number", default: "3", description: "Number of cards in the stacked deck" },
      { name: "onDone", type: "() => void", description: "Callback when clicking Mark as Done" },
      { name: "onRemindLater", type: "() => void", description: "Callback when clicking Remind Me Later" },
      { name: "onDismiss", type: "() => void", description: "Callback when dismissing the active card" },
    ],
    code: `"use client";

import React, { useState } from "react";
import { Bell, CheckCircle2, MoreHorizontal, X } from "lucide-react";
